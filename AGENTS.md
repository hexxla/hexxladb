...
**Public API at the module root:** The stable import path is **`github.com/hexxla/hexxladb`**, implemented as top-level `.go` files next to **`go.mod`** (e.g. **`db.go`**, **`tx.go`**, **`errors.go`**, **`options.go`**, **`doc.go`**, **`coord_export.go`**, tests). That matches the documented module layout (“stable API at module root”; see **`HEXXLA_DB.md`**) and is normal for Go libraries: **`internal/...`** stays private to the module, while the root package is what external callers import. A separate **`pkg/`** tree is optional and not required here.

**Modern Go (toolchain + style):** Honor the **`go`** directive in **`go.mod`** as the minimum language version. Pick language/stdlib features deliberately and read **go.dev** release notes when you adopt them. In this codebase:
...
**Docs to keep aligned with code changes:** **`docs/hexxladb/HEXXLA_DB.md`** (storage), **`docs/hexxladb/HEXXLA.md`** (memory model), **`docs/hexxladb/API_REFERENCE.md`** (exported surface), and the package comment in **`doc.go`**. Operator-facing retention/soak/incident guidance: **`docs/hexxladb/OPERATIONS.md`**. **Roadmap / out-of-scope / spec-vs-code audit:** **`docs/ROADMAP.md`**. Do not duplicate **`HEXAGONAL_ARCHITECTURE.md`**—reference it.

For editor-specific rules and skills, see **`.cursor/rules/`** and **`.cursor/skills/`** for Cursor, or **`.windsurf/rules/`** and **`.windsurf/skills/`** for Windsurf (they reference to same doc). Optional **Git** hooks: **`.pre-commit-config.yaml`** + **`make pre-commit-install`** — still run **`make ci`** before pushing; CI does not use pre-commit.
