...
**Public API at the module root:** The stable import path is **`github.com/hexxla/hexxladb`**, implemented as top-level `.go` files next to **`go.mod`** (e.g. **`db.go`**, **`tx.go`**, **`errors.go`**, **`options.go`**, **`doc.go`**, **`coord_export.go`**, tests). That matches the documented module layout (“stable API at module root”; see **`HEXXLA_DB.md`** / **`ADOPTION.md`**) and is normal for Go libraries: **`internal/...`** stays private to the module, while the root package is what external callers import. A separate **`pkg/`** tree is optional and not required here.
...
**Docs to keep aligned with code changes:** **`docs/hexxladb/HEXXLA_DB.md`** (storage), **`docs/hexxladb/HEXXLA.md`** (memory model), and the package comment in **`doc.go`**. For operator rollout context (not a build spec), see **`docs/hexxladb/ADOPTION.md`**. Do not duplicate **`HEXAGONAL_ARCHITECTURE.md`**—reference it.
