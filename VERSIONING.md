...
- **`format_version` 2** (optional **[`Options.EnableMVCC`](options.go)** on **new** files only) stores **`commit_seq`** in the header and uses version-suffixed physical keys for cells and related indexes; see [`MVCC_E2_DECISIONS.md`](docs/hexxladb/MVCC_E2_DECISIONS.md). Existing v1 files are **not** auto-upgraded.
