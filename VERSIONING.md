...
| Performance and soak | Controlled, observation, vector-scale, and placement evidence passes on release hardware; a representative bounded workload meets the adopting deployment's declared targets. | **Reproducible evidence passes on `7c94398`; the conservative five-minute, minimum-sample reference qualification passes, while adopting deployments must declare and validate material deviations from that profile** |
...
| `v0.6.0` | Read/write; explicit migration source | Read/write; migration destination | Read/write | Read/write with matching database credentials | Refused with `ErrUnsupportedFormatVersion` |
