#!/usr/bin/env bash
# Codex PostToolUse hook — warn on high-signal secret patterns in the edited file (stderr only; does not block).
# Complement with git-secrets, gitleaks in CI, or partner integrations.
# Codex PostToolUse input: {"file_path": "..."} via stdin
set -euo pipefail
