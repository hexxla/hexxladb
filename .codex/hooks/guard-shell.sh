#!/usr/bin/env bash
# Codex PreToolUse hook — runs before Bash tool when matcher matches (e.g. curl, rm, sudo).
# Asks the user to approve; adjust matcher to balance safety vs friction.
# Codex PreToolUse input: {"tool_name": "Bash", "command": "..."} via stdin
# Codex expects JSON output: {"continue": true/false, "permission": "allow/ask/deny", ...}
set -euo pipefail
