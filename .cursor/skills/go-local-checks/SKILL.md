...
Project hooks in `.cursor/hooks.json` require **`jq`** on `PATH`. They run **`gofmt -w`** after Go writes, warn on stderr for common secret patterns in edited files, gate some shell commands, and block prompts that look like pasted tokens. This does **not** replace secret scanning in CI (**gitleaks**, etc.).
