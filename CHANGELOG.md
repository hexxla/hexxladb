...
- **WAL unbounded growth** — `CommitWriteTxn` now truncates the WAL to zero after all pages are durably applied to the primary. Previously the WAL was only truncated on the next `Open`, causing it to accumulate all redo records indefinitely across transactions (25 MB for a 128 KB DB after the llm_context_engine demo). The WAL is now always zero-length between transactions.
