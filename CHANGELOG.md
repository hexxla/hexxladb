...
- **Compatibility and v1 readiness contract** — `VERSIONING.md` now records library/engine/encryption/changelog compatibility, explicit newer-format refusal, a root-API stability inventory, required pre-v1 migration notes, and evidence-backed v1 release gates. The lint/security toolchain is pinned to the vulnerability-fixed Go 1.26.6 patch and reviewed analyzer versions through one local/hosted runner, eliminating skipped PATH-only checks and `@latest` policy drift.
